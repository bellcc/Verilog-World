	...
	public Level getConfiguration(File file) {
		...
		try {
			...
			for (int i = 0; i < portList.getLength(); ++i) {
				...
				int col = Integer.parseInt(portElement.getElementsByTagName("loc-col").item(0).getTextContent());
				String portName = portElement.getElementsByTagName("port-name").item(0).getTextContent();
				boolean isInput = Boolean
						.parseBoolean(portElement.getElementsByTagName("is-input").item(0).getTextContent());
				String wireName = portElement.getElementsByTagName("wire-name").item(0).getTextContent();

				// target block
				int targetRow = Integer
						.parseInt(portElement.getElementsByTagName("target-loc-row").item(0).getTextContent());
				int targetCol = Integer
						.parseInt(portElement.getElementsByTagName("target-loc-col").item(0).getTextContent());
				String targetPortName = portElement.getElementsByTagName("target-port-name").item(0).getTextContent();
				boolean targetIsInput = Boolean
						.parseBoolean(portElement.getElementsByTagName("target-is-input").item(0).getTextContent());
				...
				if (!block.getModuleWrapper().getPortsHash().containsKey(portName)
					...
					ModulePort port = new ModulePort(block, portName, isInput, wireName, targetBlock, targetPortName,
			...
			if (worldNode.getNodeType() == Node.ELEMENT_NODE) {
				...
				String description = worldElement.getElementsByTagName("description").item(0).getTextContent();
		...
		try {
			...
			for (Block block : blockList) {
				...
				for (ModulePort port : portList) {
					...
					Element targetIsInput = doc.createElement("target-is-input");
					targetIsInput.setTextContent(String.valueOf(port.getTargetPort().getIsInput()));
					...
					targetRow.setTextContent(String.valueOf(targetBlock.getRow()));
					portElement.appendChild(targetRow);

					Element targetCol = doc.createElement("target-loc-col");
					targetCol.setTextContent(String.valueOf(targetBlock.getColumn()));
					portElement.appendChild(targetCol);

					Element targetWireName = doc.createElement("target-wire-name");
					targetWireName.setTextContent(port.getTargetPort().getWire().getName());
