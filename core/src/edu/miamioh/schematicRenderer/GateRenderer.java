...
/**
 * Renders a gate, specified by name, coordinates and color.
 * 
...
class GateRenderer {
     ...
     * 
     * @param type
     *            The Type of Gate to be rendered. Includes Ports.
     * @param x
     *            The X coordinate to draw the gate at. Corresponds to the CX of
     *            the gate.
     * @param y
     *            The Y coordinate to draw the gate at. Corresponds to the CY of
     *            the gate.
     * @param color
     *            The color of the gate.
     */
    void render(GateType type, int x, int y, Color color) {
     ...
     * 
     * @param cx
     *            Horizontal location of the center in pixels relative to the
     *            bottom left corner.
     * @param cy
     *            Vertical location of the center in pixels relative to the
     *            bottom left corner.
     * @param color
     *            The color of the gate.
     */
     ...
     * 
     * @param cx
     *            Horizontal location in pixels relative to the bottom left
     *            corner.
     * @param cy
     *            Vertical location in pixels relative to the bottom left
     *            corner.
     * @param color
     *            The color of the gate.
     */
	...
	// top curve
	// renderer.curve(cx, ty, cx + gateSize * scaleFactor / 6 + curveFactor,
	// cy + gateSize *
	// scaleFactor / 3 + curveFactor, cx + gateSize * scaleFactor / 3 +
	// curveFactor, cy +
	// gateSize * scaleFactor / 6 + curveFactor, rx, cy, 1000);
	// bottom curve
	// renderer.curve(cx, by, cx + gateSize * scaleFactor / 6 + curveFactor,
	// cy - gateSize *
	// scaleFactor / 3 - curveFactor, cx + gateSize * scaleFactor / 3 +
	// curveFactor, cy -
	// gateSize * scaleFactor / 6 - curveFactor, rx, cy, 1000);
	...
	// //Get the stage
	// Stage stage =
	// SchematicRendererScreen.getScreen().getSchematicStage();
	//
	...
	// Pixmap pixmap = new Pixmap(scaledGS, scaledGS,
	...
	//
	// Vector2[] topCurve = {new Vector2(lx, ty), new Vector2(tcx, tcy), new
	// Vector2(rx, cy)};
	// Vector2[] bottomCurve = {new Vector2(lx, by), new Vector2(bcx, bcy),
	// new Vector2(rx, cy)};
	// Vector2[] sideCurve = {new Vector2(lx, by), new Vector2(lxrc, cy),
	// new Vector2(lx, ty)};
	//
	// //draw the path
	// path = new BSpline<>();
	...
	// point = queue.pop();
	//
	// if (tempColor.set(pixmap.getPixel((int)point.x, (int)point.y)).a ==
	// 0){
	...
	renderer.curve(lx, ty, cx - gateSize * scaleFactor / 6 + curveFactor,
		cy + gateSize * scaleFactor / 3 + curveFactor,
		cx + gateSize * scaleFactor / 6 + curveFactor,
		cy + gateSize * scaleFactor / 6 + curveFactor, rx, cy, 1000);
	// bottom curve
	// renderer.arc(lx, ty, gateSize * scaleFactor, 270, 60);
	renderer.curve(lx, by, cx - gateSize * scaleFactor / 6 + curveFactor,
		cy - gateSize * scaleFactor / 3 - curveFactor,
		cx + gateSize * scaleFactor / 6 + curveFactor,
		cy - gateSize * scaleFactor / 6 - curveFactor, rx, cy, 1000);
	// side curve
	...
	renderer.curve(lx, by, lxrc, cy - gateSize * scaleFactor / 3, lxrc,
		cy + gateSize * scaleFactor / 3, lx, ty, 1000);
	...
	float lxrc = lxr + gateSize * scaleFactor / 4; // Redundant (could be
						       // just cx) but left it
						       // for consistency
	...
	// top curve
	renderer.curve(lxr, ty, cx - gateSize * scaleFactor / 6 + curveFactor,
		cy + gateSize * scaleFactor / 3 + curveFactor,
		cx + gateSize * scaleFactor / 6 + curveFactor,
		cy + gateSize * scaleFactor / 6 + curveFactor, rx, cy, 1000);
	// bottom curve
	renderer.curve(lxr, by, cx - gateSize * scaleFactor / 6 + curveFactor,
		cy - gateSize * scaleFactor / 3 - curveFactor,
		cx + gateSize * scaleFactor / 6 + curveFactor,
		cy - gateSize * scaleFactor / 6 - curveFactor, rx, cy, 1000);
	// side curve (right)
	renderer.curve(lxr, by, lxrc, cy - gateSize * scaleFactor / 3, lxrc,
		cy + gateSize * scaleFactor / 3, lxr, ty, 1000);
	// side curve (left)
	renderer.curve(lxl, by, lxlc, cy - gateSize * scaleFactor / 3, lxlc,
		cy + gateSize * scaleFactor / 3, lxl, ty, 1000);
     ...
     *
     * @param cx
     *            Center X coordinate of the frame, same as the calling gate.
     * @param cy
     *            Center Y coordinate of the frame, same as the calling gate.
     */
