...
import static edu.miamioh.schematicRenderer.SchematicRendererController.bottomEdge;
...
import static edu.miamioh.schematicRenderer.SchematicRendererController.leftEdge;
import static edu.miamioh.schematicRenderer.SchematicRendererController.rightEdge;
import static edu.miamioh.schematicRenderer.SchematicRendererController.scaleFactor;
import static edu.miamioh.schematicRenderer.SchematicRendererController.topEdge;
...
/**
 * Renders the gates, ports and wires, and adds name tags to the stage.
 * 
...
class SchematicRenderer implements Disposable {
    ...
    private int				l1Gates	 = 0;
    private boolean			compiled = false;
    
    /**
     * Creates a new SchematicRenderer. The root_tree and renderer must be set
     * before rendering.
     */
    SchematicRenderer() {
	...
	// Axis of the window where y = total height / 2
	int cxAxis = SchematicRendererController.getCurrentController()
		.getWindowHeight() / 2;
	
	// Axis of the window where x = total width / 2
	int cyAxis = SchematicRendererController.getCurrentController()
		.getWindowWidth() / 2;
	
	// frame = true;
	
	// Template
	// Draw a box to show the edges of the schematic.
	if (frame) {
	    renderer.begin(ShapeRenderer.ShapeType.Line);
	    renderer.setColor(Color.BLUE);
	    renderer.line(leftEdge, cxAxis, rightEdge, cxAxis);
	    renderer.line(cyAxis, bottomEdge, cyAxis, topEdge);
	    renderer.rect(leftEdge, bottomEdge, rightEdge - leftEdge,
		    topEdge - bottomEdge);
	    renderer.end();
	}
     ...
     * 
     * @return True if the root_tree has been visited and the schematic has
     *         built the gates and ports lists.
     */
    boolean is_Compiled() {
    ...
    /**
     * Runs the SchematicVisitor to collect Ports, Gates, and relationships from
     * the root_tree.
     */
    void getData() {
	if (root_tree != null) {
     ...
     *
     * @param type
     *            The type of gate, e.g. AND, OR, NOT, etc.
     * @param id
     *            The unique name of the port.
     * @param level
     *            The distance of the gate from the inputs.
     */
     ...
     *
     * @param id
     *            The unique name of the Input. All Inputs have only one port,
     *            an output.
     */
     ...
     *
     * @param id
     *            The unique name of the Output. All Outputs have only one port,
     *            an input.
     */
     ...
     *
     * @param lValue
     *            The gate on the left hand side.
     * @param rValue
	...
	if (gateLookup(lValue) != null) tempGl = gateLookup(lValue);
	if (portLookup(lValue) != null) tempPl = portLookup(lValue);
	
	// Get the right-hand object
	if (gateLookup(rValue) != null) tempGr = gateLookup(rValue);
	if (portLookup(rValue) != null) tempPr = portLookup(rValue);
	
	if (tempGl != null && tempGr != null) tempGr.addInput(tempGl.getID());
	if (tempGl != null && tempPr != null) tempPr.addInput(tempGl.getID());
	if (tempPl != null && tempGr != null) tempGr.addInput(tempPl.getID());
	if (tempPl != null && tempPr != null) tempPr.addInput(tempPl.getID());
    }
	...
	// Render Gates and Ports
	{
	    ...
	    /*
	     * Determine a proper scalingFactor by the greatest number of gates
	     * in either direction, divided by the gateSize and the number of
	     * gates in that direction.
	     */
	    ...
	    // Count the number of Gates vertically on the first level. The
	    // first level should set
	    // the vertical height because every other level must fit within it.
	    numOfGatesHoriz = (maxLevel + 1 > numOfGatesHoriz) ? maxLevel + 1
		    : numOfGatesHoriz;
	    for (int i = 0; i < gates.size(); i++) {
		tempG = gates.get(i);
		if (tempG.getLevel() == 1) {
		    numOfGatesVert = (i - skips > numOfGatesVert) ? i - skips
			    : numOfGatesVert;
		    l1Gates++;
		} else
		    skips++;
		...
		if (tempP.getType().equals(OUTPUT))
		    numOfGatesVert = (i - totalIns > numOfGatesVert)
			    ? i - totalIns : numOfGatesVert;
		else
		    totalIns++;
	    }
	    ...
	    // Place the gates by their level, scaled gateSize and level
	    // multiplicity.
		...
		for (int i = 0; i < gates.size(); i++) {
			...
			// Change the algorithm for setting the cy of Gates so
			// that the first
			...
			// Place following levels by the average height of their
			// inputs.
			else
			    tempG.setCY(getYCenter(tempG.getInputs()));
			
			// Add a name tag to the gate
			nx = tempG.getRX();
			ny = tempG.getRY();
		...
		if (tempP.getType().equals(OUTPUT)) {
		    ...
		    // OUTUPUTs are always placed based on the average of their
		    // inputs' heights.
	    ...
	    for (Port port : ports) {
		grenderer.render(port.getType(), port.getCX(), port.getCY(),
			port.getColor());
	    }
	    
	    // Render all the gates.
	    for (Gate gate : gates) {
		grenderer.render(gate.getType(), gate.getCX(), gate.getCY(),
			gate.getColor());
	    }
	}
	...
	// Render Gate port connections
	{
		...
		ArrayList<String> copyInputs = new ArrayList<>(
			tempGr.getInputs());
		...
		for (int s = 0; s < totalInputs; s++) {
		    ...
		    // For each input, check if it's the highest. If so, save
		    // the y.
	...
	/**
	 * for(Port port : ports){ if(port.getID().equalsIgnoreCase("clk"){
	 * for(Gate gate : gates){ if(gate.getType().equals(REG)){
	 * drawClkLine(gate.getCX(), gate.getCY()); }}}}
	 */
	
	ports.stream().filter(port -> port.getID().equalsIgnoreCase("clk"))
		.forEach(port -> gates.stream()
			.filter(gate -> gate.getType().equals(REG))
			.forEach(gate -> drawClkLine(port, gate.getCX(),
				gate.getCY())));
	...
	return (int) (leftEdge + gateSize * scaleFactor / 2
		+ level * gateSize * scaleFactor * 2);
    }
    
    private int getYCenter(int row) {
	return (int) (bottomEdge + gateSize * scaleFactor / 2
		+ row * gateSize * scaleFactor * 2);
    }
	...
	for (String id : inputs) {
	    if (gateLookup(id) != null) avgY += gateLookup(id).getCY();
	    if (portLookup(id) != null) avgY += portLookup(id).getCY();
	}
	
	if (numOfInputs != 0) avgY /= numOfInputs;
	...
	return (int) (row * (ydiff / l1Gates) + bottomEdge + scaleFactor * gateSize / 2);
    }
